...
const BroadcastVid = () => {
  ...
  const handleOffer = async (offer, targetLanguage, remoteSocketId) => {
    ...
    await peerConnection.current[remoteSocketId].setRemoteDescription(
  ...
  const initializeSocket = () => {
    ...
    socket.current.on(
      'offer',
      (offer, targetLanguage, remoteSocketId, isBroadcast) =>
        handleOffer(offer, targetLanguage, remoteSocketId),
    );
    socket.current.on(
      'answer',
      (answer, targetLanguage, remoteSocketId, isBroadcast) =>
        handleAnswer(answer, targetLanguage, remoteSocketId),
    );
    ...
    <div className="video-chat">
      ...
      <div className="video-container">
        {userRole === 'viewer' && remoteVideoRefs.length ? (
          ...
          <video
            className={
              isBroadcasting && remoteVideoRefs.length
                ? 'hide-video'
                : 'local-video'
            }
            ref={localVideoRef}
          ...
          {isBroadcasting && userRole !== 'broadcaster' ? (
            ...
            >
              <Phone
                style={{
                  color:
                    userRole === 'broadcaster' && isBroadcasting
                      ? 'red'
                      : 'green',
                }}
              />
